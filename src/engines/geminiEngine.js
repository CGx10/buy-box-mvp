...
class GeminiAnalysisEngine {
    constructor() {
        ...
        console.log('🚀 Using model: gemini-2.0-flash-lite (CORRECT MODEL)');
        ...
        if (process.env.GEMINI_API_KEY && (process.env.ENABLE_GEMINI === 'true' || process.env.ENABLE_GEMINI === undefined)) {
            try {
                ...
                this.available = true;
                console.log('✅ Gemini engine initialized successfully with model: gemini-2.0-flash-lite');
            } catch (error) {
    ...
    async analyzeUserData(userData) {
        ...
        try {
            ...
            const stage1Prompt = selectedModel === 'gemini-1.0-pro-002' 
                ? this.buildFlashStage1Prompt(userData)
                : this.buildStage1Prompt(userData);
            console.log(`🔍 Using ${selectedModel === 'gemini-1.0-pro-002' ? 'FLASH-SPECIFIC' : 'REGULAR'} Stage 1 prompt`);
            ...
            const stage2Prompt = selectedModel === 'gemini-1.0-pro-002' 
                ? this.buildFlashStage2Prompt(userData, stage1Text)
                : this.buildStage2Prompt(userData, stage1Text);
            console.log(`🔍 Using ${selectedModel === 'gemini-1.0-pro-002' ? 'FLASH-SPECIFIC' : 'REGULAR'} Stage 2 prompt`);
